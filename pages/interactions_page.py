...
class SortablePage(BasePage):
    ...
    def get_sortable_items(self, elements):
        item_list = self.element_are_all_visible(elements)
        return [item.text for item in item_list]

    def change_list_order(self):
        self.element_is_visible(self.locators.TAB_LIST).click()
        order_before = self.get_sortable_items(self.locators.LIST_ITEM)
        item_list = random.sample(self.element_are_all_visible(self.locators.LIST_ITEM), k=2)
    ...
    def change_grid_order(self):
        ...
        item_list = random.sample(self.element_are_all_visible(self.locators.GRID_ITEM), k=2)
        item_what = item_list[0]
        item_where = item_list[1]
        self.action_drag_and_drop_to_element(item_what, item_where)
...
class ResizablePage(BasePage):
    ...
    def check_size_resizable(self):
        self.action_drag_and_drop_by_offset(self.element_is_visible(self.locators.RESIZABLE_HANDLE), random.randint(1, 300), random.randint(1, 10))
        max_size = self.get_px_from_width_height(self.get_max_min_size(self.locators.RESIZABLE))
        self.action_drag_and_drop_by_offset(self.element_is_visible(self.locators.RESIZABLE_HANDLE), random.randint(-200, -1), random.randint(-10, -1))
...
class DroppablePage(BasePage):
    ...
    def drop_accept(self):
        ...
        no_acceptable_div = self.element_is_visible(self.locators.NOT_ACCEPTABLE)
        drop_div = self.element_is_visible(self.locators.DROP_HERE_ACCEPT)
        self.action_drag_and_drop_to_element(no_acceptable_div, drop_div)
        not_acceptable = drop_div.text
        self.action_drag_and_drop_to_element(acceptable_div, drop_div)
        acceptable = drop_div.text
        return not_acceptable, acceptable
    ...
    def drop_prevent_propogation(self):
        ...
        return text_not_greedy_box, text_not_greedy_inner, text_greedy_box, text_greedy_inner_box
